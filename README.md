<div align="center">

# COGITA·DISCE·NECTE·FARE

### Mathematics • Machine Learning • MLOps • Data Engineering • LLMs

I build data-driven systems at the intersection of mathematical thinking, reliable engineering and intelligent software.

[![GitHub followers](https://img.shields.io/github/followers/SOHCAHTOAbw?style=flat-square&logo=github&label=Followers)](https://github.com/SOHCAHTOAbw?tab=followers)
[![Profile views](https://komarev.com/ghpvc/?username=SOHCAHTOAbw&style=flat-square&color=2783DE)](https://github.com/SOHCAHTOAbw)

</div>

## About me

- 🔢 Interested in applied mathematics and computational problem-solving
- 🤖 Exploring machine learning and production-ready AI systems
- ⚙️ Building reproducible MLOps workflows
- 🏗️ Learning scalable data engineering architectures
- 🧠 Experimenting with large language models and intelligent applications

## Languages

<p>
  <img src="https://skillicons.dev/icons?i=python,r,cpp,java" alt="Python, R, C++, and Java" />
  <img src="https://img.shields.io/badge/SQL-336791?style=for-the-badge&logo=postgresql&logoColor=white" alt="SQL" />
</p>

## Focus areas

```text
Mathematics       → foundations, modelling, and analytical thinking
Machine Learning  → training, evaluation, and practical applications
MLOps             → reproducibility, deployment, and monitoring
Data Engineering  → pipelines, transformation, and reliable data systems
LLMs              → retrieval, agents, evaluation, and AI applications
```

## Current direction

- Developing end-to-end machine-learning projects
- Strengthening data structures, algorithms, and mathematical foundations
- Learning how to deploy and monitor models reliably
- Building data pipelines that support analytics and AI
- Exploring LLM applications, RAG systems, and evaluation

---

<div align="center">
  <sub>Learning deeply. Building carefully. Improving continuously.</sub>
</div>
